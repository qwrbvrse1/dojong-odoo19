...
class AutomationConfigurationStep(models.Model):
    ...
    def _compute_graph_data(self):
        total = self.env["automation.record.step"]._read_group(
            ...
            groupby=["configuration_step_id", "processed_on:day"],
            aggregates=["__count"],
        )
        ...
        locale = get_lang(self.env).code
        for step, processed_day, count in total:
            day_label = babel.dates.format_datetime(
                processed_day, format="dd MMM yyy", tzinfo=tz, locale=locale,
            )
            if day_label in result[step.id]["error"]:
                result[step.id]["error"][day_label] += count
        for step, processed_day, count in done:
            day_label = babel.dates.format_datetime(
                processed_day, format="dd MMM yyy", tzinfo=tz, locale=locale,
            )
            if day_label in result[step.id]["done"]:
                result[step.id]["done"][day_label] += count
            if day_label in result[step.id]["error"]:
                result[step.id]["error"][day_label] -= count
        for record in self:
