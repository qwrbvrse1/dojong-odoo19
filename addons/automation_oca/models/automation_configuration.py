...
class AutomationConfiguration(models.Model):
    ...
    def _compute_click_count(self):
        data = self.env["link.tracker.click"]._read_group(
            domain=[("automation_configuration_id", "in", self.ids)],
            groupby=["automation_configuration_id"],
            aggregates=["__count"],
        )
        mapped_data = {config.id: count for config, count in data}
        for record in self:
    ...
    def _compute_activity_count(self):
        data = self.env["automation.record.step"]._read_group(
            ...
            groupby=["configuration_id", "step_type"],
            aggregates=["__count"],
        )
        mapped_data = defaultdict(lambda: {})
        for config, step_type, count in data:
            mapped_data[config.id][step_type] = count
        for record in self:
    ...
    def _compute_record_count(self):
        data = self.env["automation.record"]._read_group(
            domain=[("configuration_id", "in", self.ids), ("is_test", "=", False)],
            groupby=["configuration_id", "state"],
            aggregates=["__count"],
        )
        mapped_data = defaultdict(lambda: {})
        for config, state, count in data:
            mapped_data[config.id][state] = count
        for record in self:
    ...
    def _compute_record_test_count(self):
        data = self.env["automation.record"]._read_group(
            domain=[("configuration_id", "in", self.ids), ("is_test", "=", True)],
    ...
    def _get_automation_records_to_create(self):
        ...
        if self.field_id:
            ...
            query.groupby = SQL('"%s"."%s"' % (Record._table, self.field_id.name))
            sql = query.select(SQL('MIN("%s".id)' % Record._table))
        else:
            sql = query.select()
