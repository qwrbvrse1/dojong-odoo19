...
(function() {
  ...
  function emitRealtimeEvent(message) {
    if (state.callbacks.onRealtimeEvent) {
      state.callbacks.onRealtimeEvent(message);
    }
  }
  ...
  function emitTranscriptUpdate(role, payload) {
    const text = extractText(payload);
    if (text && state.callbacks.onTranscriptUpdate) {
      state.callbacks.onTranscriptUpdate({
        role,
        text,
        payload
      });
  ...
  async function handleWebSocketMessage(message) {
    ...
    switch (message.type) {
      ...
      case 'rtf-user-transcription':
        emitTranscriptUpdate('user', message.payload);
        break;

      case 'rtf-bot-text':
        emitTranscriptUpdate('assistant', message.payload);
        break;

      case 'rtf-bot-started-speaking':
        updateStatus('connected', 'Dograh is speaking', 'Live voice session active');
        break;

      case 'rtf-bot-stopped-speaking':
        updateStatus('connected', 'Listening', 'Waiting for your response');
        break;
  ...
  window.DograhWidget = {
    ...
    onRealtimeEvent: (callback) => { state.callbacks.onRealtimeEvent = callback; },
    onTranscriptUpdate: (callback) => { state.callbacks.onTranscriptUpdate = callback; },
    ...
    initInline: (options) => {
      ...
      if (options.onRealtimeEvent) state.callbacks.onRealtimeEvent = options.onRealtimeEvent;
      if (options.onTranscriptUpdate) state.callbacks.onTranscriptUpdate = options.onTranscriptUpdate;
