...
class ResUsers(models.Model):
    ...
    def _compute_dojo_role(self):
        group_admin = self.env.ref("dojo_core.group_dojo_admin")
        group_instructor = self.env.ref("dojo_core.group_dojo_instructor")
        group_parent = self.env.ref("dojo_core.group_dojo_parent_student")
        for user in self:
