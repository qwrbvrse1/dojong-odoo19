...
class DojoClassSession(models.Model):
    ...
    present_count = fields.Integer(
        ...
        help="Number of enrolled students marked present or late.",
    )
    absent_count = fields.Integer(
        string="Absent",
        compute="_compute_attendance_stats",
        help="Number of enrolled students marked absent, excused, sick, injury, vacation, or other.",
    )
    attendance_rate = fields.Float(
        string="Attendance Rate (%)",
        compute="_compute_attendance_stats",
        help="Percentage of enrolled students that attended (present or late).",
    )
    ...
    @api.depends("enrollment_ids.attendance_state", "enrollment_ids.status")
    def _compute_attendance_stats(self):
        for session in self:
            registered = session.enrollment_ids.filtered(
                lambda e: e.status == "registered" and e.attendance_state != "pending"
            )
            present = len(registered.filtered(lambda e: e.attendance_state == "present"))
            absent = len(registered.filtered(lambda e: e.attendance_state in ("absent", "excused")))
            total = present + absent
            session.present_count = present
            session.absent_count = absent
            session.attendance_rate = (present / total * 100) if total else 0.0
    ...
    def action_complete(self):
        ...
        for session in self:
            if session.state != "open":
                raise UserError("Only open sessions can be completed.")
