...
# ── CRM stage name constants (mirror dojo_crm.models.crm_lead) ───────────────
_CRM_STAGE_TRIAL_BOOKED = "Trial Booked"
_CRM_STAGE_TRIAL_IN_PROGRESS = "Trial-in-progress"


class DojoCrmLeadTodos(models.Model):
    """Creates instructor ``project.task`` todos when a CRM lead moves through
    trial-related pipeline stages — so the instructor who teaches the trial
    session is kept in the loop alongside the salesperson.

    Covers three trigger points:
      1. Lead → **Trial Booked**   : instructor notified a trial is incoming
      2. Lead → **Trial-in-progress**: instructor reminded to follow up
      3. ``no_show`` set True      : instructor alerted the lead didn't show
    """
    ...
    def _get_trial_instructor_users(self):
        ...
        Falls back to all active instructors in the lead's company when no
        session is booked or the session has no assigned instructor.
        """
        self.ensure_one()
    ...
    def write(self, vals):
        # Capture leads not yet marked no-show before the write so we can
        # create instructor todos after super() (which may take an early-return
        # path for no_show stamping that bypasses a post-write hook).
        newly_no_show = (
