#!/usr/bin/env bash
# Usability-pass orchestrator. Script drives everything; Claude Code only executes stages.
...
#
# Usage:  ./scripts/usability_pass/run_usability_pass.sh        # full run, stages 0..6
#         ./scripts/usability_pass/run_usability_pass.sh 2      # resume from stage 2
set -uo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKTREE="${UPASS_WORKTREE:-/opt/worktrees/dojong-odoo19/usability-pass}"

# Main repo resolution — the toolkit can live anywhere (e.g. ~automation/usability_pass):
# 1. UPASS_REPO env var, 2. toolkit sitting inside the repo (<repo>/scripts/usability_pass),
# 3. derived from the worktree's git metadata.
if [ -n "${UPASS_REPO:-}" ]; then
  ORIG_REPO="$UPASS_REPO"
  ...
  echo "FATAL: cannot locate main repo — set UPASS_REPO=/path/to/dojong-odoo19" >&2; exit 1
fi
BRANCH="${UPASS_BRANCH:-usability-pass}"   # only used if WORKTREE must be created fresh
LOGDIR="$HERE/logs"
MAX_ATTEMPTS="${UPASS_MAX_ATTEMPTS:-3}"
...
export COMPOSE_PROJECT_NAME="${UPASS_PROJECT:-$(basename "$ORIG_REPO" | tr '[:upper:]' '[:lower:]' | tr -cd 'a-z0-9_-')}"
export UPASS_BASE_URL="${UPASS_BASE_URL:-http://localhost:8070}"
export UPASS_DB="${UPASS_DB:-odoo19}"
...
sync_working_state() {
  ...
  cp "$ORIG_REPO/USABILITY_PASS.md" "$WORKTREE/USABILITY_PASS.md" 2>/dev/null \
    || cp "$HERE/USABILITY_PASS.md" "$WORKTREE/USABILITY_PASS.md" 2>/dev/null
  # Carry over untracked files from the original checkout (e.g. seed/demo scripts)
  ...
  if ! git -C "$WORKTREE" diff --quiet || [ -n "$(git -C "$WORKTREE" status --porcelain)" ]; then
    ...
    git -C "$WORKTREE" commit -m "upass: import working state + toolkit" >/dev/null 2>&1
  fi
}
...
run_stage() {
  ...
  while [ "$attempt" -le "$MAX_ATTEMPTS" ]; do
    ...
    if [ "$attempt" -eq 1 ]; then
      ...
      ( cd "$WORKTREE" && timeout "${UPASS_STAGE_TIMEOUT:-1800}" \
    ...
    if [ $? -eq 0 ]; then
      ...
      git -C "$WORKTREE" commit -m "upass: stage $n checkpoint (gate passed)" >/dev/null 2>&1
      return 0
    fi
...
# ── Main ─────────────────────────────────────────────────────────────────────
setup_worktree
log "Project=$COMPOSE_PROJECT_NAME  Worktree=$WORKTREE  Base=$UPASS_BASE_URL  DB=$UPASS_DB"
...
git -C "$WORKTREE" tag -f "upass-$(date +%Y%m%d-%H%M)" >/dev/null 2>&1
log "ALL STAGES PASSED. Update build is in $WORKTREE (branch $BRANCH)."
log "Runbook: $WORKTREE/USABILITY_PASS_RUNBOOK.md"
